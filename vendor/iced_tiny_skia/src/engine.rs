    ...
    pub fn draw_text(
        ...
        match text {
            ...
            Text::Cached {
                ...
                // The cached text viewport is a clip, not the glyph bounds.
                // A partial menu row may paint beyond it even when the viewport
                // itself fits inside the current damage rectangle.
                let Some(clip_bounds) = clip_bounds
                    .intersection(&(*local_clip_bounds * transformation))
                else {
                    return;
...
mod tests {
    ...
    fn draw_menu_row(damage: Rectangle) -> tiny_skia::Pixmap {
        let mut pixels = tiny_skia::Pixmap::new(240, 80).unwrap();
        let mut mask = tiny_skia::Mask::new(240, 80).unwrap();
        adjust_clip_mask(&mut mask, damage);
        let row = Text::Cached {
            ...
            color: Color::WHITE,
            size: Pixels(40.0),
            line_height: Pixels(50.0),
            font: Font::DEFAULT,
            align_x: Alignment::Left,
            align_y: alignment::Vertical::Top,
            shaping: Shaping::Basic,
            // Only a thin slice of the row is visible at the menu edge.
            clip_bounds: menu_clip(),
        };
    ...
    fn assert_paint_is_confined(
        ...
        for y in 0..pixels.height() {
            for x in 0..pixels.width() {
                let alpha = pixels.pixel(x, y).unwrap().alpha();
                if x as f32 >= visible.x
                    && (x as f32) < visible.x + visible.width
                    && y as f32 >= visible.y
                    && (y as f32) < visible.y + visible.height
                {
                    painted += usize::from(alpha > 0);
                } else {
                    assert_eq!(
                        alpha, 0,
                        "cached glyph leaked outside clip at ({x}, {y})"
                    );
                }
            }
        }
        assert!(
            painted > 0,
            "test must render visible text, not an empty image"
        );
    }

    #[test]
    fn cached_menu_row_cannot_paint_below_menu_edge() {
        let pixels = draw_menu_row(Rectangle {
            x: 0.0,
            y: 0.0,
            width: 240.0,
            height: 80.0,
        });
        assert_paint_is_confined(&pixels, menu_clip());
    }

    #[test]
    fn cached_menu_row_obeys_both_menu_and_partial_repaint_bounds() {
        ...
        let pixels = draw_menu_row(damage);
        assert_paint_is_confined(
            &pixels,
            menu_clip().intersection(&damage).unwrap(),
        );
    }
}
