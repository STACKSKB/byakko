...
pub fn validate(config: &Configuration) -> Result<()> {
    ...
    for opcode in [
        crate::nia87::settings::DEBOUNCE_READ,
        crate::nia87::settings::AUTO_OS_READ,
        crate::nia87::settings::SLEEP_READ,
        crate::nia87::settings::OPTIONS_READ,
    ] {
        let Some(reply) = config.settings.raw_reply(opcode) else {
            return Err("missing settings reply".into());
        };
        if reply.len() != crate::nia87::settings::REPORT_LEN || reply[0] != opcode {
