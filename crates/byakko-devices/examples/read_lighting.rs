//! Read-only probe of the lighting command path used by the desktop.
use byakko_core::{
    lighting::{Content, editor::Status},
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let [output] = arguments.as_slice() else {
        return Err("Usage: read_lighting NEW_COMPLETION.json".into());
    };
    let mut file = OpenOptions::new()
        ...
        .open(output)?;
    let mut session =
        Session::new(nia87::descriptor())?.with_lighting(nia87::lighting_adapter::capabilities())?;
    let executor = Executor::spawn(nia87::Nia87Adapter, Default::default())?;
    executor.set_generation(session.connect()?);
    executor
        .try_submit(session.request_lighting_read()?)
    ...
    file.write_all(b"\n")?;
    file.sync_all()?;
    let read_ok = matches!(&completion, Completion::ReadLighting { result: Ok(_), .. });
    session.accept(completion);
    let editor = session.lighting().expect("configured lighting");
    ...
    match &editor.baseline().expect("verified baseline").content {
        Content::Editable(setting) => {
            println!("Verified lighting read: {setting:?}. No setters sent.")
        }
