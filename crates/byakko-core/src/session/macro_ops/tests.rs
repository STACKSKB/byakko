...
fn shared_activity_excludes_other_feature_and_rejects_wrong_slot_then_verifies() {
    ...
    let Command {
        generation,
        operation,
        payload: CommandPayload::Macro(FeatureCommand::Apply { expected, desired }),
    } = command
    else {
        unreachable!()
    };
    ...
    assert_eq!(
        session.accept(Completion {
            ...
            payload: CompletionPayload::Macro {
                slot: "wrong".into(),
                result: FeatureResult::Apply(Ok(snapshot(2, 2)))
            }
        }),
...
fn disconnected_macro_reply_is_stale_and_changed_reconnect_keeps_draft() {
    ...
    assert_eq!(
        session.accept(Completion {
            ...
            payload: CompletionPayload::Macro {
                slot,
                result: FeatureResult::Read(Ok(snapshot(1, 1)))
            }
        }),
        Acceptance::IgnoredStale
    );
...
fn failed_keymap_write_cancels_catalog_without_erasing_macro_read_failure() {
    ...
    session.accept(Completion {
        ...
        payload: CompletionPayload::Macro {
            slot,
            result: FeatureResult::Read(Err("slot read failed".into())),
        },
    });
    let Command {
