...
| Host-driven lighting | Captured actual0D music/0E screen commands; retained Windows screen/WASAPI streams passed hardware/camera/restoration checks. Iced screen-average uses a pure session lifecycle, bounded executor frames and selected-target Nia87 adapter; Stop and close require restoration, with headless failure/ordering tests. Linux sampler cross-builds. | Physical Iced screen streaming, close/focus-loss/disconnect acceptance, sustained streaming, Iced playback music, Wayland capture, Linux runtime/audio routing, X11 disconnect handling |
...
Iced screen-average lighting is implemented behind a distinct host stream
lifecycle. Physical Iced acceptance and playback music remain open. Host
