...
| Local configurations | Retained research app has full native archive capture, review and guarded apply; normal multi-section round trip passed, while first injected failure failed automatic recovery and an explicit later restore succeeded. Iced now captures, exports to a new file, and reviews a chosen native archive using forward/reverse preflight; a read-only complete capture matched the earlier baseline byte for byte. | Give native apply typed recovery outcomes before exposing it in Iced; resolve unexplained changes during failed recovery, repeat failure-injection acceptance, and validate OS GUI interaction |
